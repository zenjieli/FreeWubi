...
bool WubiDict::hasCode(const std::string &code) const {
    return dict_.count(code) > 0;
}
...
bool WubiDict::loadFrequency(const std::string &path) {
    ...
    while (std::getline(fin, line)) {
        ...
        if (it == freq_.end() || rank < it->second)
            freq_[text] = rank;
    }
    return true;
}
...
std::vector<PromptCandidate>
WubiDict::promptCandidates(const std::string &partialCode) const {
    ...
    if (it != dict_.end()) {
        for (auto &entry : it->second) {
            if (seen.insert(entry.first).second)
                result.push_back({entry.first, entry.first});
        }
    }
    ...
    for (char c = 'a'; c <= 'y'; c++) {
        ...
        for (auto &entry : it2->second) {
            ...
            prefix.emplace_back(globalFreq(entry.first), -entry.second,
                                entry.first, code);
        }
