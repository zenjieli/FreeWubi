...
EngineLogic::EngineLogic(WubiDict &dict, WubiDict &dict_common,
                         PinyinDict &pinyin)
    : dict_(dict), dict_common_(dict_common), pinyin_(pinyin) {}

bool EngineLogic::processKey(const KeyInput &input, EngineState *state,
                             IEngineOutput *out) {
    ...
    if (input.sym == Keys::Control_R) {
        ...
        if (state->englishMode &&
            (!state->code.empty() || state->literalMode ||
             state->tempPinyinMode || state->slashMode)) {
            state->code.clear();
    ...
    if (input.sym == Keys::Grave && input.isUnmodified() &&
        !state->code.empty()) {
    ...
    if (state->literalMode) {
        ...
        if (input.sym == Keys::Space && input.isUnmodified()) {
            if (state->literalModeAuto) {
                ...
                state->literalBuffer.clear();
                out->clearPanel();
            } else if (state->literalBuffer.size() <
                       static_cast<size_t>(kMaxLiteralLen)) {
        ...
        if (!input.hasAny(Modifiers::Ctrl | Modifiers::Alt | Modifiers::Super) &&
            input.sym >= Keys::Exclam && input.sym <= Keys::Asciitilde) {
            if (state->literalBuffer.size() <
                static_cast<size_t>(kMaxLiteralLen)) {
    ...
    if (state->slashMode) {
        ...
        if ((input.sym == Keys::Space || input.sym == Keys::Return) &&
            input.isUnmodified()) {
    ...
    if (input.hasAny(Modifiers::Shift)) {
        return false;
    }
    ...
    if (input.sym == Keys::Space && input.isUnmodified() &&
        ...
        if (state->code == "z" && !state->lastCommit.empty()) {
            ...
            auto dictCandidates =
                activeDict(state).promptCandidates(state->code);
            auto allCandidates =
                mergeCustomPhrases(state->code, dictCandidates);
    ...
    if (input.sym == Keys::Equal && input.isUnmodified() &&
        ...
        auto candidates =
            activeDict(state).promptCandidates(state->code);
        int maxPage =
            (static_cast<int>(candidates.size()) - 1) / pageSize_;
        if (state->pageOffset < maxPage) {
            ++state->pageOffset;
            updateUI(state, out);
        }
    ...
    // Punctuation keys
    if (input.sym == Keys::Comma || input.sym == Keys::Period ||
        input.sym == Keys::Exclam || input.sym == Keys::Question ||
        input.sym == Keys::Semicolon || input.sym == Keys::Colon ||
        input.sym == Keys::Less || input.sym == Keys::Greater ||
        input.sym == Keys::Underscore || input.sym == Keys::Backslash ||
        input.sym == Keys::Asciicircum) {
    ...
    if (input.sym == Keys::z && input.isUnmodified() &&
        state->code.empty()) {
    ...
    if (input.isUnmodified() && input.lowercase()) {
        ...
        if (static_cast<int>(state->code.size()) == kMaxCodeLen) {
            ...
            int customCount =
                (it != customPhrases_.end()) ? it->second.size() : 0;
...
void EngineLogic::updateUI(EngineState *state, IEngineOutput *out) {
    ...
    std::string preeditText =
        state->rareMode ? ("`" + state->code) : state->code;
    ...
    if (state->code == "z" && !state->lastCommit.empty()) {
        ...
        if (!allCandidates.empty()) {
            ...
            int count = std::min(
                pageSize_, static_cast<int>(allCandidates.size()) - start);
            if (start >= static_cast<int>(allCandidates.size())) {
                start = 0;
                count = std::min(pageSize_,
                                 static_cast<int>(allCandidates.size()));
                state->pageOffset = 0;
            }
            std::vector<CandidateEntry> cands;
            for (int i = 0; i < count; i++) {
                cands.push_back(
                    {allCandidates[start + i].text,
                     std::to_string(i + 1) + "." +
                         allCandidates[start + i].label});
            }
            out->setCandidates(cands);
        }
    }
...
void EngineLogic::commitTopCandidate(EngineState *state, IEngineOutput *out,
...
void EngineLogic::updatePinyinUI(EngineState *state, IEngineOutput *out) {
    ...
    if (!state->pinyinCode.empty()) {
        ...
        if (!exact.empty()) {
            int start = state->pageOffset * pageSize_;
            int count = std::min(pageSize_,
                                 static_cast<int>(exact.size()) - start);
            if (start >= static_cast<int>(exact.size())) {
                start = 0;
                count =
                    std::min(pageSize_, static_cast<int>(exact.size()));
                state->pageOffset = 0;
            }
            std::vector<CandidateEntry> cands;
            for (int i = 0; i < count; i++) {
...
std::string EngineLogic::chinesePunct(EngineState *state,
    ...
    if (ascii == "\"") {
        std::string result = state->quoteOpenDouble
                                 ? "\xe2\x80\x9c"  // "
                                 : "\xe2\x80\x9d"; // "
    ...
    if (ascii == "'") {
        std::string result = state->quoteOpenSingle
                                 ? "\xe2\x80\x98"  // '
                                 : "\xe2\x80\x99"; // '
...
WubiDict &EngineLogic::activeDict(EngineState *state) {
    if (!state->rareMode && dict_common_.numCodes() > 0)
        return dict_common_;
    return dict_;
}

void EngineLogic::setCustomPhrases(
...
std::vector<CandidateEntry> EngineLogic::mergeCustomPhrases(
...
static std::string takeFirst(const std::string &code, size_t n) {
    return code.substr(0, std::min(n, code.size()));
}

std::string
EngineLogic::computePhraseCode(const std::string &phrase) const {
    ...
    if (n == 1) {
        ...
        result = takeFirst(codes[0], 1) + takeFirst(codes[1], 1) +
                 takeFirst(codes[2], 2);
    } else {
        // 4+ chars: first key of char 1, 2, 3, and last
        result = takeFirst(codes[0], 1) + takeFirst(codes[1], 1) +
                 takeFirst(codes[2], 1) + takeFirst(codes[n - 1], 1);
    }
