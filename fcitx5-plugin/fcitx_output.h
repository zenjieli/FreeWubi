...
class FcitxOutput : public IEngineOutput {
 public:
  FcitxOutput(fcitx::InputContext* ic, int pageSize) : ic_(ic), pageSize_(pageSize) {}

  void commit(const std::string& text) override { ic_->commitString(text); }
  ...
  void setCandidates(const std::vector<CandidateEntry>& candidates) override {
    auto candList = std::make_unique<fcitx::CommonCandidateList>();
    candList->setPageSize(pageSize_);
    for (auto& c : candidates) {
      candList->append<fcitx::DisplayOnlyCandidateWord>(fcitx::Text(c.label));
    }
    if (!candidates.empty()) {
  ...
  void updateStatus() override { ic_->updateUserInterface(fcitx::UserInterfaceComponent::StatusArea); }
