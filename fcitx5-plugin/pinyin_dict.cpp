...
bool PinyinDict::loadFromFile(const std::string& path) {
  ...
  std::sort(entries_.begin(), entries_.end(),
            [](const PinyinEntry& a, const PinyinEntry& b) { return a.stripped < b.stripped; });

  return !entries_.empty();
}

std::vector<PinyinEntry> PinyinDict::lookup(const std::string& prefix) const {
  auto cmp = [](const PinyinEntry& e, const std::string& s) { return e.stripped < s; };
  auto lo = std::lower_bound(entries_.begin(), entries_.end(), prefix, cmp);

  std::vector<PinyinEntry> result;
  for (auto it = lo; it != entries_.end() && it->stripped.compare(0, prefix.size(), prefix) == 0; ++it) {
    result.push_back(*it);
  }

  std::sort(result.begin(), result.end(), [](const PinyinEntry& a, const PinyinEntry& b) { return a.freq < b.freq; });
