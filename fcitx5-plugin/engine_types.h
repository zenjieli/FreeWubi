...
inline Modifiers operator|(Modifiers a, Modifiers b) {
    return static_cast<Modifiers>(static_cast<uint32_t>(a) |
...
struct KeyInput {
    ...
    KeyInput() : sym(0), modifiers(Modifiers::None) {}
    KeyInput(uint32_t s, Modifiers m = Modifiers::None)
        : sym(s), modifiers(m) {}

    // Printable ASCII character, or 0
    char asciiChar() const {
        return (sym >= 0x21 && sym <= 0x7E) ? static_cast<char>(sym) : 0;
    }
    // Lowercase letter a-z, or 0
    char lowercase() const {
        return (sym >= 0x61 && sym <= 0x7A) ? static_cast<char>(sym) : 0;
    }
    // Uppercase letter A-Z, or 0
    char uppercase() const {
        return (sym >= 0x41 && sym <= 0x5A) ? static_cast<char>(sym) : 0;
    }
    bool isUnmodified() const { return modifiers == Modifiers::None; }
