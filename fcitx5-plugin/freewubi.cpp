...
FreeWubiEngine::FreeWubiEngine(fcitx::Instance *instance)
    : instance_(instance),
      logic_(dict_, dict_common_, pinyin_) {
    instance_->inputContextManager().registerProperty("freewubiState",
                                                      &stateFactory_);
    loadDictionary();
...
void FreeWubiEngine::loadDictionary() {
    std::string path = fcitx::StandardPath::global().locate(
        fcitx::StandardPath::Type::PkgData,
        "data/wubi86_jidian.dict.yaml");
    ...
    if (dict_.loadFromFile(path)) {
        FCITX_INFO() << "FreeWubi: loaded " << dict_.numCodes()
                     << " codes from " << path;
    } else {
...
void FreeWubiEngine::loadCommonDictionary() {
    ...
    if (dict_common_.loadFromFile(path)) {
        FCITX_INFO() << "FreeWubi: loaded " << dict_common_.numCodes()
                     << " codes from common dict " << path;
    } else {
...
void FreeWubiEngine::loadPinyin() {
    ...
    if (pinyin_.loadFromFile(path)) {
        FCITX_INFO() << "FreeWubi: loaded " << pinyin_.numEntries()
                     << " pinyin entries from " << path;
    } else {
        FCITX_WARN() << "FreeWubi: cannot open pinyin " << path;
    }
}

void FreeWubiEngine::keyEvent(const fcitx::InputMethodEntry & /*entry*/,
    ...
    KeyInput input(keyEvent.key().sym(),
                   static_cast<Modifiers>(
                       static_cast<uint32_t>(keyEvent.key().states())));
...
std::string
FreeWubiEngine::subModeLabelImpl(const fcitx::InputMethodEntry & /*entry*/,
...
std::string
FreeWubiEngine::subModeIconImpl(const fcitx::InputMethodEntry & /*entry*/,
                                fcitx::InputContext &ic) {
    auto *state = ic.propertyFor(&stateFactory_);
    if (state->engine.englishMode) {
...
void FreeWubiEngine::reset(const fcitx::InputMethodEntry & /*entry*/,
...
const fcitx::Configuration *FreeWubiEngine::getConfig() const {
    return &config_;
}
...
void FreeWubiEngine::syncConfigToEngine() {
    ...
    auto symForKey = [](TriggerKey key) -> uint32_t {
        switch (key) {
        case TriggerKey::BracketLeft:  return Keys::BracketLeft;
        case TriggerKey::BracketRight: return Keys::BracketRight;
        case TriggerKey::Slash:        return Keys::Slash;
        case TriggerKey::Semicolon:    return Keys::Semicolon;
        }
        return 0;
    };
    ...
    if (te == se || te == tp || se == tp) {
        ...
        config_.tempEnglishKey.setValue(TriggerKey::BracketLeft);
        config_.secondTempEnglishKey.setValue(TriggerKey::Slash);
        config_.tempPinyinKey.setValue(TriggerKey::BracketRight);
        te = Keys::BracketLeft;
        se = Keys::Slash;
